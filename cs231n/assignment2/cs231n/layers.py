...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    cache = eps, gamma, x_std, x, sample_mean, sample_var
...
def batchnorm_backward(dout, cache):
    ...
    """
    eps, gamma, x_std, x, sample_mean, sample_var = cache
    N, D = x.shape
...
def max_pool_forward_naive(x, pool_param):
    ...
    for n in range(N):  # n-th input sample
        for c in range(C):  # c-th in-depth slice
            for i in range(H_out):
                x_rf_tl = i * stride
                x_rf_tr = x_rf_tl + pool_height
                for j in range(W_out):
                    x_rf_bl = j * stride
                    x_rf_br = x_rf_bl + pool_width
                    out[n, c, i, j] = np.max(x[n, c, x_rf_tl:x_rf_tr, x_rf_bl:x_rf_br])
        ...
        for c in range(C):  # c-th in-depth slice
            for i in range(H_out):
                ...
                for j in range(W_out):
                    x_rf_bl = j * stride
                    x_rf_br = x_rf_bl + pool_width
                    x_rf = x[n, c, x_rf_tl:x_rf_tr, x_rf_bl:x_rf_br]
                    x_rf_argmax = np.unravel_index(x_rf.argmax(), x_rf.shape)

                    dx[n, c, x_rf_tl + x_rf_argmax[0], x_rf_bl + x_rf_argmax[1]] += dout[n, c, i, j]
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    x_roll = np.rollaxis(x, 1, 4)
    x_rool_reshape = x_roll.reshape(N * H * W, C)
    out_tmp, cache = batchnorm_forward(x_rool_reshape, gamma, beta, bn_param)
    out = np.rollaxis(out_tmp.reshape(x_roll.shape), 3, 1)
...
def spatial_batchnorm_backward(dout, cache):
    ...
    dout_roll = np.rollaxis(dout, 1, 4)
    dout_rool_reshape = dout_roll.reshape(N * H * W, C)
    dx_tmp, dgamma, dbeta = batchnorm_backward(dout_rool_reshape, cache)
    dx = np.rollaxis(dx_tmp.reshape(dout_roll.shape), 3, 1)
